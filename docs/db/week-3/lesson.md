...
The catch block in `db.query` always has an error parameter. If the SQL errors then the callback function passed to the `catch` method is invoked with an error object generated by node postgres.
