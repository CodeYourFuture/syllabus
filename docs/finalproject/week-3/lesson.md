...
- Roughly equal pull requests (PRs). That is, we do not expect everyone to open the exact same number of PRs or commit the same number of lines of code, but we do expect features to be fairly evenly shared. As a quick guide: in a group of four, given 25% as the  no one should be committing more than **30%** of the code and no one should be committing less than **20%**.
